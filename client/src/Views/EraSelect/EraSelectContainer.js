...
class EraSelectContainer extends Component {
  ...
  handleWheel = (e, current) => {
    ...
    if (
      ...
      (current === this.state.eras.length && scrollDirection === 1)
    ) {
  ...
  render() {
    const { eras, from, to, current } = this.state;
    if (eras.length === 0) {
    ...
    return (
      eras.length > 0 && (
        <Fragment>
          <Spring
            from={{
              bgcolor: this.state.eras[from].section_bgcolor
            }}
            to={{
              bgcolor: this.state.eras[to].section_bgcolor
            }}
          >
            {(springProps) => (
              <Fragment>
                <Parallax
                  ...
                  style={{
                    backgroundColor: springProps.bgcolor
                  }}
                  pages={eras.length}
                  horizontal
                  scrolling={false}
