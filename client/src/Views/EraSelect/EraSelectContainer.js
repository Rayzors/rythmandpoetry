...
class EraSelectContainer extends Component {
  ...
  handleWheel = (e, current) => {
    ...
    if((scrollDirection === -1 && current === 1) || (current === this.state.eras.length + 1 && scrollDirection === 1) ) {
  ...
  render() {
    ...
    return eras.length > 0 && (
      <Fragment>
        <Spring
        from={{ bgcolor: eras[from] ? eras[from].section_bgcolor : "#5e0821" }}
            ...
            <Fragment>
              ...
              >
                {eras.map((era, i) => (
                  ...
                  <ParallaxLayer
                    offset={ eras.length }
                    onWheel={ (e, eras) => this.handleWheel(e, this.state.eras.length + 1) }
                  >
                    <HallOfFame />
                  </ParallaxLayer>
              </Parallax>
